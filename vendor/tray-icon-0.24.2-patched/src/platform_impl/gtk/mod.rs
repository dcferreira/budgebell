...
impl TrayIcon {
    pub fn new(id: TrayIconId, attrs: TrayIconAttributes) -> crate::Result<Self> {
        ...
        // `set_icon_full` expects an icon-theme *name*, resolved against the
        // theme path set above — not a full path with extension. Passing the
        // full path here means every StatusNotifierItem watcher (including
        // GNOME's appindicator extension) fails its icon-theme lookup and
        // silently never renders anything, even though the indicator itself
        // still registers on D-Bus as Active.
        indicator.set_icon_full(&icon_name(&icon_path), "icon");
    ...
    pub fn set_icon(&mut self, icon: Option<Icon>) -> crate::Result<()> {
        ...
        self.indicator
            .set_icon_full(&icon_name(&icon_path), "tray icon");
        self.path = icon_path;
...
/// 3. `/tmp/tray-icon`
/// The bare icon-theme name for a temp icon file — its filename without the
/// extension, resolved relative to whatever `set_icon_theme_path` was given.
fn icon_name(icon_path: &Path) -> String {
    icon_path
        .file_stem()
        .map(|stem| stem.to_string_lossy().into_owned())
        .unwrap_or_else(|| icon_path.to_string_lossy().into_owned())
}
