...
/// meeting, and Do Not Disturb / Focus each independently gate every trigger.
...
impl QuietState {
    ...
    pub fn is_quiet(&self) -> bool {
        self.idle || self.in_meeting || self.dnd
    }
}
...
mod tests {
    ...
    fn any_single_active_source_makes_the_state_quiet() {
        // Given three states, each with exactly one quiet source active
        let idle = QuietState {
            idle: true,
